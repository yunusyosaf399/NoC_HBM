...
#set_false_path -from [get_pins -filter {REF_PIN_NAME=~C*} -of_objects [get_cells -hierarchical -filter {NAME =~ "*axis_mem/inst/we_4_ext_reg_reg*"}]] -to [get_pins -filter {REF_PIN_NAME=~D*} -of_objects [get_cells -hierarchical -filter {NAME =~ "*axis_mem/inst/trace_data_ack_reg[0]*"}]]
